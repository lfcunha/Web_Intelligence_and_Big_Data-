...
def mapfn(k, v):
    ...
    for lines in v.splitlines():
        a= [(x+"-"+''.join(ch for ch in y.rstrip().strip("-") if ch.isalnum()), 1) for x in lines.split(":::")[1].split("::") for y in lines.split(":::")[2].split() if y not in stopw]
        for a in a:
            yield a
